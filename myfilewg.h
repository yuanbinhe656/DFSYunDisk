...
public:
    ...
    //上传文件处理，取出上传任务队列的队首任务，上传完成后，再取下一个任务
    ...
    void clearItems();
    // 添加上传文件项目item
    void addUploadItem(QString iconPath=":/upload.png",QString name = "上传文件");
    // 文件item展示
    void refreshFileItem();

    // =====显示用户的文件列表======
    // desc 降序 asc 升序 normal普通用户，PvASC按升序 pvDesc按下载量降序
    ...
    // 获取用户文件列表
    void getUserFileList(Display cmd = Normal);
...
protected:
    void contexMenuEvent(QContextMenuEvent *event);
...
private:
    ...
    long M_userFileCount;  // 用户文件数据
    int m_start;   // 文件位置起点
    int m_count;   //每次请求文件个数

    // 定时器
    QTimer m_uploadFileTimer;  // 定时检查上传队列是否有任务上传
