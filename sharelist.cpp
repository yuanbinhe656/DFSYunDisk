...
// ====== 文件item显示========
// 清空文件列表
void ShareList::clearsharFileList()
{
...
void ShareList::addDownloadFiles()
{
    ...
    if( p == NULL)
    {
        cout << "DownlaodTask::getInstance() == NULL";
        return ;
    }
...
// 设置json包
QByteArray ShareList::setShareFileJosn(QString user, QString md5, QString filename)
{
    /*
        {
            "user": "yoyo",
...
void ShareList::dealFilePv(QString md5, QString filename)
{
    ...
    */
    QByteArray data = setShareFileJosn(login->getUser(),  md5, filename); // 设置json包
...
// 处理选中的文件
void ShareList::dealSelectdFile(CMD cmd)
{
    // 获取当前选中的item
    QListWidgetItem *item = ui->listWidget->currentItem();
    if(item == NULL)
...
void ShareList::cancelSharefile(FileInfo *info)
{
    ...
    */
    QByteArray data = setShareFileJosn(login->getUser(),  info->md5, info->filename); // 设置json包
